...
if [ -f "devops.tf" ]; then
  ...
  TF_VAR_sumologic_access_id="${INPUT_SUMO_LOGIC_ACCESS_ID}" TF_VAR_sumologic_access_key="${INPUT_SUMO_LOGIC_ACCESS_KEY}" TF_VAR_sumologic_access_key="${INPUT_SUMO_LOGIC_ENVIRONMENT}" terraform apply -auto-approve ${INPUT_APPLY_PARAMETERS} || exit 4
else
